...
- 하나의 정점으로부터 시작해서 차례대로 모든 정점들을 한 번씩 방문하는 것.
- 루트 노드에서 시작해서 다음 분기(branch)로 넘어가기 전에 해당 분기를 완벽하게 탐색하는 방법.
- 주로 모든 노드를 방문하고자하는 경우에 DFS를 사용한다.
### DFS의 특징
- 자기 자신을 호출하는 순환 알고리즘의 형태를 가지고 있다.
- BFS와 마찬가지로 어떤 노드를 방문했었는지 여부를 반드시 검사해야한다.
- DFS 구현 방식은 "순환 호출(재귀) 이용"과 "스택 사용"의 2가지 방법이 있다.
- 스택은 먼저 들어 온 데이터가 나중에 나가는 형식인 FILO의 자료구조로 삽입은 append, 삭제는 pop을 통해 마지막에 들어온 것부터 차례대로 빠진다.
- 특정 노드와 연결된 다른 노드를 찾으면 그 중 하나를 골라 이동하고, 다시 또 이동한 곳과 연결된 노드들 중 하나를 골라 이동하는 방식이 반복되는 구조이다.
- 만약, 현재 노드에서 더 이상 이동할 노드가 없다면, 이전 노드로 되돌아간다.(백트래킹) 

### Stack을 이용한 DFS의 동작 과정
![image](https://github.com/Sangh0Yoon/Tech_Interview_preparation/assets/100823955/e3e31325-a6df-43eb-82c7-ec487ac25581)
1. 시작점을 1번 노드라고 할 때, 이를 스택에 삽입하고 방문 처리를 한다.
   - 스택 상태 : 1
2. 1번 노드와 연결된 2,3,8중에 하나를 골라(여기서는 낮은 번호 우선) 삽입하고 방문 처리한다.
   - 스택 상태 : 1 2
3. 2번 노드와 연결된 노드는 1,7인데 방문하지 않은 노드는 7이므로 스택에 삽입하고 방문 처리한다.
   - 스택 상태 : 1 2 7
4. 7번 노드와 연결된 2,6,8중 방문하지 않고 낮은 번호인 6을 스택에 삽입하고 방문 처리한다.
   - 스택 상태 : 1 2 7 6
5. 6번 노드에서는 더이상 방문하지 않은 노드가 없으므로 스택에서 빼고 6의 부모노드인 7번 노드로 되돌아간다(백트래킹)
   - 스택 상태 : 1 2 7
6. 7번 노드에서 방문하지 않은 노드는 8밖에 남지 않았으므로 삽입 후 방문 처리한다.
   - 스택 상태 : 1 2 7 8
7. 위의 탐색 방식을 모든 노드를 전부 방문할 때까지 반복한다.
8. 최종적으로 노드의 탐색 순서는 1 -> 2 -> 7 -> 6 -> 8 -> 3 -> 4 -> 5 이다.
### 재귀 함수를 이용한 DFS 동작 과정
1. 파라미터로 넘어온 정점이 이미 방문한 정점일 경우 return 하도록 기본 조건을 설정한다.
2. 파라미터로 넘어온 정점이 방문하지 않은 정점일 경우 방문 표시를 한다.
3. 인접 정점에 대해 재귀적으로 함수를 호출하며 탐색을 진행한다.
### DFS에 재귀함수가 사용되는 이유
- 가장 중요한 키워드는 "함수 호출과 스택 메모리" 이다.
- OS에 메모리 공간에는 스택 메모리라는 함수가 사용하는 메모리 공간이 존재한다. 즉, 함수 안에서 또 함수를 호출하면 스택 메모리에 재귀적으로 호출된 함수들의 메모리 공간이 계속해서 쌓이게 된다.
- 즉, 가장 먼저 불린 함수는 자신의 내부에서 호출된 함수들이 모두 return할 때까지 OS 스택 메모리 최하단에서 자신의 메모리를 유지한채 기다리게 된다.
- 이렇게 되면 스택을 활용한 DFS와 같이 가장 가장 안쪽의 함수를 먼저 수행하고 그 결과를 가지고 제일 처음 시작한 함수의 연산을 수행하는 흐름을 만들 수 있다.
- 즉, for문으로 DFS와 같은 코드 흐름을 만들기 위해서는 개발자가 stack이라는 자료구조를 만들어 필요한 정보들을 저장하는 등의 복잡한 절차를 거쳐야 하지만, 재귀로 구현하면 이 절차를 운영체제가 자동으로 해준다는 장점이 있기 때문에 재귀함수로 DFS를 구현하면 편리하다.
### DFS 시간 복잡도
- DFS 하나당 N번의 loop을 돌게 되므로 O(n)의 시간복잡도를 가진다. 근데 N개의 정점을 모두 방문해야하므로 n*O(n) -> O(N^2)의 시간 복잡도를 가지게 된다.
### DFS 코딩 문제
- 프로그래머스 Lv.2 타겟 넘버
### BFS와 DFS
- BFS에서는 Queue의 FIFO 성질을 이용해서 인접리스트에서 depth가 가장 낮은 노드를 가지고 오는 것.
- DFS는 Stack의 FILO 성질을 이용해 인접리스트에서 depth가 가장 깊은 노드를 가지고 오는 것.
- 즉, BFS는 넓이를 우선시해서 특정 노드와 연결된 모든 노드를 한번씩 거친 다음 다음 노드로 이동하는 방식이고, DFS는 깊이를 우선시하므로 한 노드의 자식노드들만 계속 깊이 파고드는 방식이다.
---------------------------------------
# 기술 면접 예상 질문
## 자료 구조
